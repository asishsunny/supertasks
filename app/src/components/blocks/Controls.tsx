...
<div className="flex gap-0 h-8 items-center relative shrink-0 w-full">
  <div className="bg-ui-bg-segment-control flex gap-0.5 items-center p-0.5 rounded-lg shrink-0">
    {tabs.map((tab) => (
      <button
        key={tab.key}
        className={`${activeTab === tab.key ? "bg-ui-bg-base text-ui-fg-base shadow-elevation-card-rest" : "text-ui-fg-subtle"} px-2.5 py-1 rounded-md txt-compact-small cursor-pointer`}
  ...
  <div className="flex gap-2 items-center relative shrink-0">
    ...
    <div className="relative">
      <Input type="search" size="small" placeholder={searchPlaceholder} onChange={(e) => onSearch?.(e.target.value)} />
      {searchShortcut && <Kbd className="absolute right-1.5 top-1/2 -translate-y-1/2">{searchShortcut}</Kbd>}
    </div>
  </div>
</div>
  );
