...
<div className="flex flex-col h-full overflow-clip relative rounded-[12px] shadow-[0px_0px_0px_1px_rgba(0,0,0,0.08),0px_8px_16px_0px_rgba(0,0,0,0.08),0px_16px_32px_0px_rgba(0,0,0,0.08)] shrink-0 max-w-[480px] w-full">
  ...
  <div className="flex flex-1 flex-col gap-6 min-h-[1px] pb-6 pt-4 px-6 relative w-full">
    ...
    <div className="flex flex-col gap-4 relative shrink-0 w-full">
      ...
      {infoRows?.map((row, i) => (
        <div key={i} className="flex items-center justify-between relative shrink-0 w-full txt-compact-small">
          <p className="relative shrink-0 text-ui-fg-subtle">
            {row.label}
          </p>
          {row.type === "status" && row.statusColor && (
            <div className={`flex gap-0.5 h-5 items-center justify-center px-1.5 py-px relative rounded-full shrink-0 border`} style={{ backgroundColor: row.statusColor.bg, borderColor: row.statusColor.border }}>
          ...
          {row.type === "date" && (
            <p className={`relative shrink-0 ${row.overdue ? "text-ui-fg-error" : "text-ui-fg-base"}`}>
              {row.value}
            </p>
          )}
        </div>
