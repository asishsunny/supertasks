...
<div className="flex gap-4 items-start relative shrink-0 w-full">
  {charts.map((chart, chartIndex) => (
    <div key={chartIndex} className="bg-ui-bg-base flex flex-1 flex-col gap-4 min-w-[1px] overflow-clip p-6 relative rounded-xl shadow-elevation-card-rest">
      ...
      <div className="flex flex-col gap-4 relative shrink-0 w-full">
        {chart.rows.map((row, rowIndex) => (
          <div key={rowIndex} className="flex flex-col gap-1 relative shrink-0 w-full">
            <div className="flex gap-2 items-center relative shrink-0 w-full">
              ...
              <div className="bg-ui-border-base flex-1 h-2 min-w-[1px] overflow-clip relative rounded">
                <div className={`absolute h-2 left-0 rounded top-0`} style={{ backgroundColor: row.color, width: chart.total > 0 ? `${(row.count / chart.total) * 100}%` : '0%' }} />
              </div>
