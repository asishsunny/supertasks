...
// ── Component keys (variant keys, not set keys) ─────────────────────────────
const COMP = {
...
// Nav items to keep and their icon instance-swap IDs
const NAV_ICONS = {
  Dashboard: '4071:4623',
  Tasks:     '4071:2407',
  Team:      '4071:2411',
...
// Extensions — show Settings at bottom, hide everything else
for (const child of top?.children || []) {
  ...
  if (hasSettings) {
    ...
    for (const item of child.children || []) {
      ...
      const iconKey = Object.keys(item.componentProperties || {}).find(k => /^Icon/i.test(k));
      if (iconKey) item.setProperties({ [iconKey]: '4071:2415' });
...
// Hide bell icon, swap cog to moon
